...
def distance_based_loss(logits, locs):
    ...
    dst_matrix = haversine_dist(lat1, lng1, lat2, lng2)/1000
...
def train(train_dataloader, model, optimizer, epoch, device, temperature = 0.07, loss_func = loss_function):
    ...
    for i ,(imgs, locs) in bar:
        ...
        indices = torch.randint(0, model.loc_gallery.size(0), (4000,))
        rnd_locs = model.loc_gallery[indices].to(device)
        locs_all = torch.cat([locs, rnd_locs]).to(device)

        img_embeddings, loc_embeddings = model(imgs, locs_all)
